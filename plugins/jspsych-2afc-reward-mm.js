/**
 * jspsych-2afc-reward-mm
 * 
 * Show two stimuli and give reward afterwards.
 * Reward is deterministic and depends on the chosen stimulus.
 */


jsPsych.plugins['2afc-reward-mm'] = (function () {

    var plugin = {};

    jsPsych.pluginAPI.registerPreload('2afc-reward-mm', 'stimuli', 'image');

    plugin.info = {
        name: '2afc-reward-mm',
        description: 'Present 2-AFC and give reward afterwards.',
        parameters: {
            title: {
            ...
            feedback_stimuli: {
                type: jsPsych.plugins.parameterType.OBJECT,
                pretty_name: 'list of 3 HTML that are the feedback for each stimulus',
                default: {left: "good", right: "bad", timeout: "timeout"},
                description: 'an object containing fields "left", "right" and "timeout" with any valid html'
            },
            responses: {
                type: jsPsych.plugins.parameterType.OBJECT,
            ...
            max_decision_time: {
                ...
                default: 1700,
                description: 'Maximum time for making decision (in ms). , `Infinity` is allowed (no time limit).'
            },
    ...
    plugin.trial = function (display_element, trial) {
        ...
        html += `<div id="jspsych-2afc-reward-stimulus-wrapper" 
                      style="display:flex;
                             align-items: center;
                             justify-content: center;">
                        <div id="jspsych-2afc-reward-left-stimulus" 
                             class="jspsych-2afc-reward-stimulus"
                             style="border:${ bord_style };
                                    width: ${ trial.width };
                                    visibility:hidden;">
                             ${ trial.stimuli.left }
                        </div>
                        <div id="jspsych-2afc-reward-feedbackstim" 
                             style="visibility:hidden;">
                        </div>
                        <div id="jspsych-2afc-reward-fixcross" 
                             style="width: ${ trial.distance }">
                             ${ trial.fixation_cross }
                        </div>
                        <div id="jspsych-2afc-reward-right-stimulus" 
                             class="jspsych-2afc-reward-stimulus"
                             style="border:${ bord_style };
                                    width: ${ trial.width };
                                    visibility:hidden;">
        ...
        var after_fixcross = function(){
            ...
            el=display_element.querySelector(`#jspsych-2afc-reward-fixcross`);
            el.style["visibility"]="hidden";
            display_element.querySelectorAll(".jspsych-2afc-reward-stimulus").forEach( (box) => {
            ...
            if( !(trial.max_decision_time === Infinity) ){
        ...
        var after_feedback = function(){
            ...
            var waitfor=Math.max( (trial.total_trial_duration-elapsed), 0);
            jsPsych.pluginAPI.setTimeout(end_trial, waitfor);
        }

        var show_feedback = function(stim){
            console.log(stim);
            trial_data["feedback_onset"]=performance.now()-start_time;
            display_element.querySelectorAll(".jspsych-2afc-reward-stimulus").forEach( (box) => {
                box.style["visibility"]="hidden";
            })
            el=display_element.querySelector(`#jspsych-2afc-reward-feedbackstim`);
            el.style["visibility"]="visible";
            el.innerHTML=trial.feedback_stimuli[stim];
        ...
        var after_response_or_timeout = function(info) {
            if( typeof info == "undefined" ){
               ...
               show_feedback("timeout");
                ...
                el=display_element.querySelector(`#jspsych-2afc-reward-${ selected_side }-stimulus`);
                ...
                setTimeout(removeBorder, trial.highlight_duration, selected_side);
            }
        }; // after_response
